...
```{r}
sample_data <-read.csv("Sample_data_tom.csv")
...
pheno_data <- pheno_data[colnames(exprs_data),]
...
filtered_exprs_lna <- lfq_protein_tidy_lna %>%
  mutate(sample=paste0(Treatment, '_', Condition, '_', Replicate)) %>%
...
# column names first
treatment <- pData(lfq_protein[,colnames(filtered_exprs_lna)])$Treatment

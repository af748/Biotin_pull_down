---
title: "2024_02_19_ASO_Biotin_pulldown_Data_processing"
...
treatment <- factor(treatment, levels = c('NCL', 'CL'))
...
table(deqms_results_lna$sca.adj.pva<0.01)
...
table(deqms_results_ion$sca.adj.pva<0.01)
...
table(deqms_results_veh$sca.adj.pva<0.01)
...
lfq_protein_tidy_ION_LNA <- lfq_protein %>%
  # make the MSnSet into 'tidy' format for further testing
  biobroom::tidy.MSnSet(addPheno=TRUE) %>% # addPheno=TRUE adds the phenotype columns
  subset(sample=="CL_ION_1"| sample=="CL_ION_2" | sample=="CL_ION_3" | sample=="CL_LNA_1"| sample=="CL_LNA_2" | sample=="CL_LNA_3") %>%
  mutate(Treatment = ifelse(sample == "CL_ION_1", "ION",
                      ifelse(sample == "CL_ION_2", "ION",
                      ifelse(sample == "CL_ION_3", "ION", 
                      ifelse(sample == "CL_LNA_1", "LNA",
                      ifelse(sample == "CL_LNA_2", "LNA",
                      ifelse(sample == "CL_LNA_3", "LNA",
                      "N/A"))))))) %>%
  filter(is.finite(value)) %>%
  group_by(protein) %>%
  filter(n()>=2) %>% 
...
pheno_data2<- rownames_to_column(pheno_data, var = "sample")

pheno_data2 <- pheno_data2 %>% mutate(Treatment = ifelse(sample == "CL_ION_1", "ION",
                      ifelse(sample == "CL_ION_2", "ION",
                      ifelse(sample == "CL_ION_3", "ION", 
                      ifelse(sample == "CL_LNA_1", "LNA",
                      ifelse(sample == "CL_LNA_2", "LNA",
...
treatment2 <- factor(treatment2, levels = c('LNA', 'ION'))
...
deqms_results_ion_lna_cl <- outputResult(efit_deqms_ion_lna_cl, coef_col=2)
```
...
table(deqms_results_ion_lna_cl$sca.adj.pva<0.05)

deqms_results_ion_lna_cl %>% filter(logFC>0) %>% head()

deqms_results_veh %>%
  ggplot(aes(x = logFC, y = -log10(sca.P.Value), colour = sca.adj.pval < 0.05)) +
  geom_point() +
  theme_camprot(border=FALSE, base_size=15) +
  scale_colour_manual(values = c('grey', get_cat_palette(2)[2]), name = 'ION vs LNA Sig.') +
  labs(x = 'Ion vs LNA (Log2)', y = '-log10(p-value)')
```
...
lfq_protein_tidy_ION_VEH <- lfq_protein %>%
  # make the MSnSet into 'tidy' format for further testing
  biobroom::tidy.MSnSet(addPheno=TRUE) %>% # addPheno=TRUE adds the phenotype columns
  subset(sample=="CL_ION_1"| sample=="CL_ION_2" | sample=="CL_ION_3" | sample=="CL_VEH_1"| sample=="CL_VEH_2" | sample=="CL_VEH_3") %>%
  mutate(Treatment = ifelse(sample == "CL_ION_1", "ION",
                      ifelse(sample == "CL_ION_2", "ION",
                      ifelse(sample == "CL_ION_3", "ION", 
                      ifelse(sample == "CL_VEH_1", "VEH",
                      ifelse(sample == "CL_VEH_2", "VEH",
                      ifelse(sample == "CL_VEH_3", "VEH",
                      "N/A"))))))) %>%
  filter(is.finite(value)) %>%
  group_by(protein) %>%
  filter(n()>=2) %>% 
...
deqms_results_ion_veh_cl <- outputResult(efit_deqms_ion_veh_cl, coef_col=2)
```
...
table(deqms_results_ion_veh_cl$sca.adj.pva<0.05)
...
deqms_results_ion_veh_cl %>%
  ...
  labs(x = 'Ion vs VEH (Log2)', y = '-log10(p-value)')
```
...
lfq_protein_tidy_LNA_VEH <- lfq_protein %>%
  # make the MSnSet into 'tidy' format for further testing
  biobroom::tidy.MSnSet(addPheno=TRUE) %>% # addPheno=TRUE adds the phenotype columns
  subset(sample=="CL_LNA_1"| sample=="CL_LNA_2" | sample=="CL_LNA_3" | sample=="CL_VEH_1"| sample=="CL_VEH_2" | sample=="CL_VEH_3") %>%
  mutate(Treatment = ifelse(sample == "CL_LNA_1", "LNA",
                      ifelse(sample == "CL_LNA_2", "LNA",
                      ifelse(sample == "CL_LNA_3", "LNA", 
                      ifelse(sample == "CL_VEH_1", "VEH",
                      ifelse(sample == "CL_VEH_2", "VEH",
...
pheno_data2 <- pheno_data2 %>% mutate(Treatment = ifelse(sample == "CL_LNA_1", "LNA",
                      ifelse(sample == "CL_LNA_2", "LNA",
                      ifelse(sample == "CL_LNA_3", "LNA", 
                      ifelse(sample == "CL_VEH_1", "VEH",
                      ifelse(sample == "CL_VEH_2", "VEH",
...
treatment2 <- pheno_data2$Treatment
treatment2 <- factor(treatment2, levels = c('VEH', 'LNA'))
replicate <- pheno_data2$Replicate
...
deqms_results_lna_veh_cl <- outputResult(efit_deqms_lna_veh_cl, coef_col=2)
```
...
table(deqms_results_lna_veh_cl$sca.adj.pva<0.05)
...
deqms_results_lna_veh_cl %>%
  ...
  labs(x = 'LNA vs VEH (Log2)', y = '-log10(p-value)')
```